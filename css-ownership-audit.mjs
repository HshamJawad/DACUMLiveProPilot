...
function selectorsOf(css) {
  ...
  for (const ch of css) {
    if (ch === '{') {
      const head = buf.trim();
      buf = '';
      // An at-rule head opens a block of rules, not a rule itself.
...
const owners = new Map();      // selector -> Set(file)
const present = [];
...
for (const f of FILES) {
  ...
  present.push(f);
  for (const sel of selectorsOf(fs.readFileSync(p, 'utf8'))) {
...
console.log(`Distinct selectors  : ${owners.size}`);
console.log(`Split ownership     : ${split.length}`);
