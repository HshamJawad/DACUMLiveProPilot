...
function _showAIErrorModal(errorMessage, kind) {
  ...
  const title  = isOffline ? 'AI Service Unavailable'
                           : (kind === 'clustering' ? 'Clustering Failed' : 'Criteria Generation Failed');
  ...
  const bodyText = isOffline
    ? 'The AI backend server is currently offline or unreachable.<br><br>' +
      'Your clusters, tasks and criteria have not been changed.'
  ...
  const tips =
    ...
    '<li>Select tasks and create clusters manually below</li>' +
    '<li>Write the Range and criteria directly in each cluster card</li>' +
    '<li>Use the <strong>?</strong> buttons for the formatting rules</li>' +
    '</ul></div>';
