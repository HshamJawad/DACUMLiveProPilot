...
export async function exportToWord() {
    ...
    // through as an ordinary background. Cell text stays black.
            ...
            try {
                ...
                // sheet gets written from the CONTENT that ended up in the
                // manual instead of from the STANDARD the panel agreed on.
                // Once that happens, editing an exercise silently detaches
                // the qualification from its occupational standard and
                // nothing in the document reveals it.
                //
                // So nothing here is authored. Every row is pc.text copied
                // verbatim out of Competency Clustering, carried through
                // learningOutcomesData.linkedCriteria — the same objects
                // the Module Mapping section above prints. Re-ordering data
                // that already exists is what makes drift structurally
                // impossible rather than merely discouraged.
                //
                // Gated on modules: assessment is a property of the module
                // a learner is enrolled in, not of a loose outcome. A
                // project with no modules emits nothing here and therefore
                // a byte-identical document to previous builds.
                ...
                if (_apModules.length > 0) {
                    /* Column geometry declared ONCE and fed to both the cells
                       and the table's columnWidths. docx@7.8.2 builds
                       <w:tblGrid> from columnWidths alone; omit it and the
                       grid is written as 100 twips per column, which under
                       <w:tblLayout w:type="fixed"/> overrides every tcW and
                       collapses the table. */
                    const _apCols = [5260, 1814, 1997];

                    children.push(new Paragraph({ children: [new PageBreak()], bidirectional: _rtl() }));

                    children.push(new Paragraph({
                        children: [new TextRun({ text: _t('expAssessmentPlan'), bold: true, size: 32 })],
                    ...
                    children.push(new Paragraph({
                        children: [new TextRun({ text: _t('expAssessmentPlanNote'), italics: true, size: 20 })],
                        spacing: { after: 300 },
                        bidirectional: _rtl(),
                    }));
                    ...
                    _apModules.forEach(module => {
                        ...
                        (module.learningOutcomes || []).forEach(lo => {
                            ...
                            const _apHead = (txt, i) => new TableCell({
                                children: [new Paragraph({
                                    children: [new TextRun({ text: txt, bold: true, size: 20, __shaded: true })],
                                    /* No w:jc for column 0: inside a bidi
                                       paragraph OOXML reads w:val="right" as
                                       END, which lands at the LEFT edge. Left
                                       unset, the paragraph sits at its natural
                                       start edge in both directions. */
                                    ...(i === 0 ? {} : { alignment: AlignmentType.CENTER }),
                                    bidirectional: _rtl(),
                                })],
                                width: { size: _apCols[i], type: WidthType.DXA },
                                shading: { fill: _tblFill(), type: ShadingType.CLEAR, color: 'auto' },
                            });
                            ...
                            const rows = [new TableRow({
                                tableHeader: true,
                                children: [
                                    _apHead(_t('expAssessmentCriterion'), 0),
                                    _apHead(_t('expCriterionSource'), 1),
                                    _apHead(_t('expVerdict'), 2),
                                ],
                            })];

                            /* Criteria are NOT de-duplicated across outcomes.
                               Pattern C can link one criterion to two
                               outcomes, and each outcome is assessed in its
                               own context — dropping the second occurrence
                               would leave that outcome with no evidence to
                               be judged against. */
                            pcs.forEach(pc => {
                                rows.push(new TableRow({
                                    children: [
                                        new TableCell({
                                            children: [new Paragraph({
                                                children: [new TextRun({ text: pc.text || '', size: 20 })],
                                                                bidirectional: _rtl(),
                                            })],
                                            width: { size: _apCols[0], type: WidthType.DXA },
                                        }),
                                        new TableCell({
                                            children: [new Paragraph({
                                                children: [new TextRun({
                                                    text: _tf('expCriterionSourceValue', { c: pc.clusterNumber, id: pc.id }),
                                                    size: 18,
                                                })],
                                                alignment: AlignmentType.CENTER,
                                                bidirectional: _rtl(),
                                            })],
                                            width: { size: _apCols[1], type: WidthType.DXA },
                                        }),
                                        /* Deliberately blank: filled in by hand
                                           on the printed sheet. A checkbox glyph
                                           was rejected — the Arabic PDF path uses
                                           a simple TrueType face that may not
                                           carry U+2610, and the two formats must
                                           not disagree about this column. */
                                        new TableCell({
                                            children: [new Paragraph({ text: '', bidirectional: _rtl() })],
                                            width: { size: _apCols[2], type: WidthType.DXA },
                                        }),
                                    ],
                                }));
                            });
                        ...
                        // One signature block per module: the module is the
                        // unit a learner passes or repeats.
                        children.push(new Paragraph({
